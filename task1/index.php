...
<body>
    ...
    <div id="updateBugModal" class="fixed inset-0 flex items-center justify-center bg-green-900 bg-opacity-50 hidden">
        <div class="bg-white rounded-lg p-8 max-w-md">
            <h2 class="text-2xl font-bold mb-4">Bug Updated</h2>
            <p class="text-lg">Your bug <span id ="updatedBug"></span> has been updated successfully.</p>
        </div>
    </div>
    ...
    <script>
        ...
        const evtSource = new EventSource("notify.php");
        evtSource.onmessage = function(event) {
            var updatedBugs = JSON.parse(event.data);
            var existingBugs = JSON.parse(localStorage.getItem('bugs')) || [];
            updatedBugs.forEach(function(updatedBug) {
                if (existingBugs.includes(updatedBug)) {
                    document.getElementById('updatedBug').innerText = updatedBug;
                    document.getElementById('updateBugModal').classList.remove('hidden');
                    setTimeout(function() {
                        document.getElementById('updateBugModal').classList.add('hidden');
                    }, 8000);
                }
            });
        };
